...
%% perturbation parameters:
 ...
 % adjustment timescales (yrs):
   p.tau_pert = 15.0e6;    % gaussian/step
   p.tau_log  = 0.1e6;    % logistic
 % perturbation mid-points (yrs):
   p.t0     = 25.0e6;      % gaussian/step
   p.t0_log = 2.0e6;      % logistic
...
%% surface weathering parameters:
 ...
 % factors partitioning H+ equivalents:
   p.f_py_CaCO3_0  = 0.0;       % fraction of pyrite H+ to carbonate weath
   p.f_py_CaSiO3_0 = 1.0;       % fraction of pyrite H+ to silicate weath
...
%% initial conditions, state variables:
 % concentrations:
   ...
   p.SO4_init  = 2000;                  % initial [SO4] [µmol/kg]
...
%% initial conditions, fluxes:
 % input fluxes:  
   p.J_volc0   = 15.0;       % volcanic CO2 flux [TmolC/yr]
   p.J_MOR0    = 1.6;       % mid-ocean ridge CO2 flux [TmolC/yr]
   p.J_w_sil0  = 8.0;       % silicate weathering flux [TmolC/yr]
   p.J_w_carb0 = 11.0;      % carbonate weathering flux [TmolC/yr]
   p.J_w_org0  = 8.0;       % orgC weathering flux [TmolC/yr]
   p.J_w_py0   = 5.0;       % pyS weathering flux [TmolS/yr]
   p.J_w_ev0   = 1.0;       % evapS weathering flux [TmolS/yr]
   p.J_red_v0  = 0.0;
 
 % output fluxes:
   p.J_b_carb0  = 24.0;     % carbonate burial flux [TmolC/yr]
   ...
   p.J_b_org0   = 8.0;      % orgC burial flux [TmolC/yr]
   p.J_b_py0    = 5.0;      % pyS burial flux [TmolS/yr]
